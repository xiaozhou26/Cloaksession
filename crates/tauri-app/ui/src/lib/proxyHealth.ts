/**
 * Live proxy health for profile cards. Wraps `proxy.detectGeo`
 ...
 */
import { proxy as proxyApi } from "./ipc";
...
export function useProxyHealth(
  ...
  useEffect(() => {
    ...
    wanted.set(profileId, { proxy, cc: proxyCountry ?? undefined, key });

    const cached = cache.get(profileId);
    const fresh =
      cached && cached.key === key && cached.ts > 0 && Date.now() - cached.ts < TTL;
    if (!fresh && !inFlight.has(profileId)) {
      void probe(profileId, proxy, proxyCountry ?? undefined);
    }
  ...
  const recheck = useCallback(() => {
    if (proxy && !inFlight.has(profileId)) {
      ...
      const lastCc = (h && "cc" in h ? h.cc : undefined) ?? proxyCountry ?? undefined;
  ...
  const health: ProxyHealth =
    cached && cached.key === key ? cached.health : { status: "checking", cc: proxyCountry ?? undefined };
